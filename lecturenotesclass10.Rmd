...
marbles
# c command to combine things together to a vector
```
...
```{r simulate_draws}
draw_results <- sample(marbles, size = 100000, replace = TRUE) # replace = FALSE is default
```
...
two_roll_results <- two_roll_results %>%
  mutate (event1 = roll_1 ^2 == roll_2) %>%
  mutate (event2 = abs(roll_1 - roll_2)==4)
two_roll_results
```
  
```{r summarize_rolls}
two_roll_results %>%
  summarize(prop_event1 = mean(event1),
            prop_event2 = mean(event2))
```

=> event 2 is more likely than event1
